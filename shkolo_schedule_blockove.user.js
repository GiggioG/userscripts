...
// @author       Gigo_G
// @updateURL    https://github.com/GiggioG/userscripts/raw/main/shkolo_schedule_blockove.user.js
// @downloadURL  https://github.com/GiggioG/userscripts/raw/main/shkolo_schedule_blockove.user.js
//
...
function log(...args){
    console.log("[shkolo-schedule-blockove]", ...args);
}
    ...
    for(const list of lists){
        ...
        for(let i = 1; i <= 7; i+= 2){
            if(classesTheSame(classes[i], classes[i+1])){
                classes[i  ].classList.add("shkolo-schedule-blockove-block-top");
                classes[i+1].classList.add("shkolo-schedule-blockove-block-bottom");
            }
        }
    }
    log("schedule merged.");
...
function waitForMerge() {
    if (!tab_schedule.classList.contains("shkolo-schedule-blockove-merged")) {
        waitfor_diaryShow_interval = setInterval(function() {
            ...
            if(!!table && !loading){ // bug when it merges on the old date before the new one's schedule loads
                ...
                merge(table);
                tab_schedule.classList.add("shkolo-schedule-blockove-merged");
            }
        }, 500);
    }
}
...
function launch(){
    GM_addStyle(`
div.shkolo-schedule-blockove-block-top:not(.taken-shi) {
    border-bottom-color: white;
}
div.shkolo-schedule-blockove-block-top.taken-shi {
    border-bottom-color: #abdb97;
}

div.shkolo-schedule-blockove-block-bottom:not(.taken-shi) {
    border-top-color: white;
}
div.shkolo-schedule-blockove-block-bottom.taken-shi {
    ...
    if(weekSelector){
        weekSelector.addEventListener("click", ()=>{
            tab_schedule.classList.remove("shkolo-schedule-blockove-merged");
            clearInterval(waitfor_diaryShow_interval);
            waitForMerge();
        })
    }
